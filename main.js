...
function update(){
	//TODO: Check for collision. If Player collides, fight begins. If Enemies collide, the become stronger.
...
function isCollidingWithChar(x, y){
	for(var i = 0; i < allCharacters.length; i++){
		...
		if(x+20 > e.x && x < e.x+20 && y >= e.y && y < e.y+20){
			return true;
			break;
		}
	}
...
function createLevel(){
    ...
    for(var i = 0; i < randNr; i++){
        var randX = Math.floor((Math.random() * 25))*20;
...
function spawnEnemies(){
    ...
    for(var i = 0; i < randNr; i++){
        ...
        while(isColliding(randX,randY,20,20) || isCollidingWithChar(randX,randY)){
            randX = Math.floor((Math.random() * 25))*20;
